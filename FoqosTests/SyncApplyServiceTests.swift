...
final class SyncApplyServiceTests: XCTestCase {
  ...
  func testMalformedPresentPhysicalKeysRejectWholeRecordWithoutMutationOrRepair() throws {
    for exists in [true, false] {
      let profile = BlockedProfiles(name: "Local", syncVersion: 2)
      profile.physicalKeys = ProfilePhysicalKeys(stopNFC: [PhysicalKey(name: "Spare", value: "X")])
      if exists {
        context.insert(profile)
        try context.save()
      }
      let before = SyncedProfile(from: profile, originDeviceId: deviceId)
      let record = before.toCKRecord(in: zoneID)
      record["physicalKeysData"] = Data("broken".utf8)
      record["name"] = "Incoming"
      record["version"] = 3
      record["stopNFCTagId"] = "Y"
      let service = makeService()
      XCTAssertEqual(service.applyFetchedModification(record, isPendingDeleteOrTombstoned: noPendingDelete), .ignored)
      XCTAssertTrue(service.drainReenqueues().isEmpty)
      XCTAssertNil(store.systemFields(for: profile.id.uuidString))
      XCTAssertFalse(context.hasChanges)
      if exists {
        XCTAssertTrue(SyncPayloadEquality.profilesPayloadEqual(before, SyncedProfile(from: profile, originDeviceId: deviceId)))
