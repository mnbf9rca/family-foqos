...
- Lock-code restriction checks must use `appModeManager.currentMode == .child`; the pattern `!= .parent` is forbidden. *(No lock/mode logic is changed here — but see MAINTAINER DECISION 2, which is deliberately deferred to B1.)*
...
- **MERGE GATE (B1 ↔ #203):** per [MAINTAINER DECISION 2](#maintainer-decision-2--203-child-mode-deletion-of-a-locked-profile-task-3-cross-cuts-b1), **Task 3 must not merge before B1 lands** *unless* Task 3 ships the `currentMode == .child && profile.isLocked` defer-guard itself. Task 3 turns a stuck-shields bug into a clean child unblock, which is a lock-bypass for a *locked* profile. Confirm B1's status (or add the guard) before merging D2.
...
| `FoqosTests/SharedDataSessionIdentityTests.swift` | **NEW.** The clobber regression: extension session survives an in-app stop of a different session. | 1 |
| `Foqos/Utils/StrategyManager.swift` | `startRemoteSession` routes through `activateSession` instead of setting `activeSession` directly. | 2 |
...
## MAINTAINER DECISIONS (⚠️ UNRESOLVED — surfaced, not settled)

These decisions change *what* the tasks implement. **Each task is written against the recommended option so the plan is executable, but the implementing session MUST get maintainer sign-off before merging the affected task.** If a non-recommended option is chosen, the task notes what changes.

### MAINTAINER DECISION 1 — #203: stop-immediately vs run-to-natural-end (Task 3)

When a remote `SyncedProfile` deletion arrives for a profile that currently owns the active blocking session, what should the receiving device do with the session?

- **(A) Stop the session immediately, then delete — ✅ RECOMMENDED (Task 3 implements this).** Mirrors the existing `ProfileSessionRecord`-deletion branch (`stopSessionForDeletedRecord`) exactly, and is crash-safe (clears the zombie `@Model` and drops the shields before the profile is deleted). Consequence: a parent deleting a profile on device A instantly unblocks device B (the block vanishes with the profile). **Atomicity caveat (verified):** the stop is *not* part of the profile-delete transaction — `ManualBlockingStrategy.stopBlocking` calls `session.endSession()` + `context.save()` + `deactivateRestrictions()` *before* `deleteProfile`/`commit()` run. If the delete then throws, the catch's `modelContext.rollback()` leaves a self-healing husk (profile still present, its session already ended + persisted, shields already down, `activeSession` already nil in-memory, a `FailedApply` queued for §5.6 retry). This is acceptable (retry re-drives `deleteLocalProfile` idempotently) but is **not** all-or-nothing — do not describe it as atomic.
- **(B) Let the session run to its natural end, then delete.** Preserves the in-progress block, but requires *deferring* the local profile delete while a session is live (holding a to-be-deleted profile), which complicates I12 tombstone bookkeeping and the §5.6 failed-apply retry, and re-opens the zombie window for the deferral duration. Only defensible if "a remote delete must never cut a block short" is a product requirement.
- *Recommendation:* **A.** The fetched deletion is authoritative; there is no clean way to keep a deleted profile's block alive without a parallel deferral path (which I1/I12 were designed to avoid). No live users exist (project memory), so there is no in-flight-session data to preserve.

### MAINTAINER DECISION 2 — #203: child-mode deletion of a *locked* profile (Task 3, cross-cuts B1)

Should a **child**-mode device honor a remote delete of a **locked** profile that is actively blocking, given that honoring it unblocks the child?

> **⚠️ This is a MERGE GATE, not merely a flagged decision.** Task 3 does **not** "match current behavior" for this case — it *changes* it. **Today**, a child device receiving a remote delete of a locked, actively-blocking profile runs `deleteProfile` but never calls `deactivateRestrictions()`, so the shields **stay up** (the child stays blocked — in a buggy zombie/crash-prone state, but blocked). **After Task 3**, `stopRemoteSession` deactivates restrictions → the shields **drop** → the child is cleanly, immediately unblocked. So Task 3 converts an accidental lock-*preservation* into a reliable lock-*bypass* triggerable by a delete from any same-account device (the transport carries no origin-trust signal). **D2 must therefore not merge before this is resolved.**

- **(A) Honor all remote deletes uniformly.** Simple; Task 3 stops+deletes regardless of mode/lock. But it *strengthens* the lock-bypass described above.
- **(B) On child devices, refuse/defer a remote deletion of a *locked* profile with an active session; surface a conflict.** Preserves the lock guarantee, but adds mode/lock logic to the sync-apply path and leaves a husk profile until the block ends.
- **(C) Require the delete to originate from a parent/verified device before honoring on a child.** Strongest, but needs an origin-trust signal the transport does not currently carry.
- *Recommendation / merge gate:* this is lock-gating policy that intersects the B1 lock-code work (`b1-lockcode-gating-audit`). **Ship Task 3 only when EITHER (i) B1 has landed and its lock-gating covers the remote-deletion path, OR (ii) Task 3 adds a `appModeManager.currentMode == .child && profile.isLocked` defer-guard** (refuse the teardown+delete, surface a conflict) **as part of this bundle.** Do **not** silently add mode logic to `SyncApplyService` outside one of those two sanctioned paths — flag it, get the maintainer decision, and enforce the sequencing. (If (ii) is chosen, use `currentMode == .child` — never `!= .parent`, per AGENTS.md.)

### MAINTAINER DECISION 3 — #237: behavior on identity mismatch (Task 1)

When an in-app `endSession`/`flush` finds the shared active session belongs to a *different* session id (extension-created), what should happen to that foreign shared session?

- **(A) Leave it fully untouched (no-op) — ✅ RECOMMENDED (Task 1 implements this).** Safest under I1; the extension remains the authoritative owner and its own append-safe teardown (`endActiveSharedSession`) / the `getAndFlushCompletedSessionsForScheduler` drain handles its lifecycle. Risk: a stale shared session could linger until the next extension callback (bounded; the extension self-heals on its next start/stop).
- **(B) End it append-safely via `endActiveSharedSession` so its `endTime` is captured.** Captures the foreign session's end, but means the app ends a session it does not own — reintroducing a milder clobber and risking a double-append if the ids ever coincide.
- *Recommendation:* **A.** The defect is *destruction* of a foreign session; a no-op stops the destruction. Capturing the foreign end is the extension's job.
...
- [ ] **Step 3: Refresh every `file:line` in Tasks 1–3** to the post-C2 line numbers. Use symbol search (function names), not raw line numbers — names are stable, lines are not.

- [ ] **Step 4: Boot the simulator once and run the full suite green** to establish a clean baseline before changing anything (see Running tests). If it is not green on fresh `main`, STOP and report — do not build on a red baseline.

No commit for Task 0 (it is verification only). Proceed to Task 1.

---

## Task 1: #237 — SharedData session-identity gating
...
git commit -m "fix(#237): gate SharedData session mutators by session identity
...
closing their read-then-write TOCTOU. Leaves a foreign shared session
untouched (MAINTAINER DECISION 3-A)."
```
...
## Final integration step (run after all three tasks)

- [ ] **Full suite green.**

Run: `xcodebuild test -project FamilyFoqos.xcodeproj -scheme FamilyFoqos -destination 'platform=iOS Simulator,id=<UUID>' | xcpretty`
Expected: 0 failures. Confirm the three new/edited suites are included.

- [ ] **swift-format lint clean:** `swift-format lint --recursive .` → no output.

- [ ] **Request code review** (AGENTS.md requirement) before merging. In the PR description, explicitly list **MAINTAINER DECISIONS 1, 2, 3** and their chosen options, and request confirmation of each. Note that **MD2 (child-mode locked-profile deletion) is deferred to B1** and Task 3 ships the uniform-honoring bug fix only.

---

## Self-review (spec coverage / placeholders / type consistency)

- **Coverage:** #203 → Task 3 (wiring in Step 1 + real teardown in Step 4b); #204 → Task 2; #237 → Task 1. Every re-triage "surviving gap" maps to a task. ✅
...
- **Task 3 lock-bypass → MD2 elevated to a MERGE GATE** (B1-first or ship the child-lock defer-guard); corrected the false "matches current behavior" framing.
...
- **Residual (#237, MD3-A):** on identity mismatch the app leaves the foreign shared session untouched; it may linger until the extension's next `start`/`stop`, which self-heals it (append-safe). Bounded, non-destructive.
- **Residual (#203, MD1-A):** a remote profile deletion instantly ends the local block. Intended (the fetched deletion is authoritative); flagged for maintainer sign-off.
- **Deferred (#203, MD2):** child-mode locked-profile deletion policy is a B1 concern; Task 3 does not add mode/lock logic.
