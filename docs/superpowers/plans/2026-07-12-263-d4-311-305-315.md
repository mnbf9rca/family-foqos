...
Per the epic: **D4 and #311 after D3+E2**; **#305/#315 wherever the sync lane reaches them**. #315 is the higher-severity sync fix (high) — prefer it before #305 (medium) if the lane is contended. The four mini-plans are code-independent. D4 ships first as its own bundle/PR; #311/#305/#315(+#329) follow together on one branch/PR after D4 merges (branch from the then-current main; each mini-plan's Task-0 citation refresh absorbs the drift). **#329 rides inside the #315 mini-plan** (same sync lane, same watermark store) and is implemented **with** #315, not sequentially after it — its fetched-delete write site (Task 315.5) reuses the store built in Task 315.1 and the gates added in Tasks 315.3/315.4, so it must land in the same commit sweep as #315's tasks.
...
```bash
grep -n "currentSession.startTime != remoteStartTime" Foqos/Utils/StrategyManager.swift   # #226 scheduled reconcile (~1096; non-scheduled guard also at ~775)
grep -n "func cancelAllNotifications\|removeAllPendingNotificationRequests" Foqos/Utils/TimersUtil.swift  # #227 (~279/281)
grep -n "self.scheduleReminder(profile: sess.blockedProfile)" Foqos/Utils/StrategyManager.swift  # #242 (~745)
...
**Files:**
- Modify: `Foqos/Utils/StrategyManager.swift` (the `.alreadyActive` block in `syncScheduleSessions`, ~lines 1093-1098)
- Test: `FoqosTests/StrategyManagerScheduledReconcileTests.swift` (new)

**Interfaces:**
- Consumes: `SharedData.SessionSnapshot.blockedProfileId: UUID` (already used at `:1081`); `BlockedProfileSession.blockedProfile.id: UUID`.
...
- [ ] **Step 3: Add the helper (Step 1 code) and wire it into `syncScheduleSessions`.** Replace the reconcile guard (~lines 1093-1098):
...
- Modify: `Foqos/Utils/StrategyManager.swift` (`scheduleReminder` ~1373, `scheduleBreakReminder` ~1388 pass stable identifiers)
...
- [ ] **Step 4: Pass stable identifiers from the schedulers.** In `StrategyManager.swift`, `scheduleReminder` (~1381) — pass the profile-scoped identifier:
...
And `scheduleBreakReminder` (~1395):
...
**Files:**
- Modify: `Foqos/Utils/StrategyManager.swift` (`emergencyUnblock` closure, ~lines 740-746)
...
- [ ] **Step 3: Remove the redundant closure calls.** Replace the `emergencyUnblock` closure (`:740-746`):
...
- Modify: `Foqos/Utils/StrategyManager.swift` (session-end deferred migration, ~lines 914-928)
