...
**Roles.** Device **P** (Parent) and Device **C** (Child), same iCloud account, sync enabled on both, both running the **Debug** probe build.

- [ ] **Step 1 — Verify probes live.** On each device: manual "Sync Now" → confirm `[#307 … Sync requested: state=…]` appears in logs.
- [ ] **Step 2 — Enable-after-attach restart (exercises H1-stop / start / H4).** Device C: Settings → toggle sync **off**, then **on** (`SettingsView.swift:126`). Expect an `isEnabled TRANSITION -> false` then `-> true` pair, a `stop() ENTER` (caller = the sink), then a `state -> bootstrapping`.
- [ ] **Step 3 — Concurrent edit (A3′ window).** Within ~2s of C's re-enable (before its `startupTask` reaches `.steady`), Device P renames a profile and Device C edits a *different* profile. Forces `requestSync` on C while `state` is mid-restart.
- [ ] **Step 4 — Account-change path (H1 FAVOURITE — the step the naïve protocol omits).** On Device C: Settings → iCloud → **sign out**, then **sign back in** to the same account (or switch account and back). This is the **only** trigger of `handleAccountChange` (`handle(_:)` `.accountChange` at `:275`). Expect `EVENT .accountChange kind=…` then a `state -> disabled caller=…handleAccountChange…` with **no** preceding `isEnabled TRANSITION -> false`.
- [ ] **Step 5 — Post-account-change sync.** After Step 4, foreground Device C twice (fires `FoqosApp.swift:150-156` → `syncNow` → `requestSync`). Expect `Sync requested: state=disabled, driverNil=false` while `isEnabled==true` — the reproduced symptom.
- [ ] **Step 6 — Steady baseline.** Idle both 60s, foreground C once more to capture a steady-state read.
