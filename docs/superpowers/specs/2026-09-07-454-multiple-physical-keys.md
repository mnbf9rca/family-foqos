...
Design for issue #454. Base: `main` at `3034c03`. Revision 6, with the reviewer's two wording corrections after design approval of revision 5 at `ec9f697`: the re-upload paragraph enqueues once for every absent-field apply and credits payload equality, not a self-echo filter, for stopping repeats. Revision 5 carried the orchestrator's final sync rules of September 7, 2026 and the reviewer's remaining points from round 2. Changes from revision 4: a nil single id no longer deletes keys (an explicit empty list is the only deletion); the fresh or reinstalled device case is stated; the transport is stated as established from code and CloudKit's documented policy, with a failing device check blocking the merge rather than downgrading to a release note. Changes from revision 3: new writers always send the list field; a record without it is an old app's write, so the reader keeps its local list, applies only the single id (a changed id becomes the primary key and spares stay), and marks the profile for re-upload. Changes from revision 2: device check 33 expects retained keys with matching disabled, and Decision 5 says a nil single id means no configured key. Changes from revision 1: key identity is the value, not a generated UUID; the sync rule compares the single id with the first list value, not list membership; the test list covers start matching, QR parity, validation, migration and malformed input.
...
- The list field, when present, is authoritative for which keys exist. An explicit empty list is the only way a slot's keys are deleted through sync; a new writer produces it when the user removes every key.
- The single id decides the primary key. If it equals the first value of `base`, nothing changed in this slot. If it differs, an old app scanned a different tag for this slot: that tag becomes the primary key, keeping its name if it was already on the list, and the other keys stay as spares. New writers always set the single id to the first value, so this branch is reached only by an old app's edit.
- A nil single id says nothing about spares and deletes nothing. It arises for a slot that never had a key, where `base` is empty anyway. An old app cannot clear a single id, and a new app writes the single id from the list, so a nil single id next to a non-empty list is not a state any writer produces; the reader keeps the list and the setter mirrors the first value back on the next local save.

For a supported profile schema, a present but undecodable incoming `physicalKeysData` blob is rejected before any profile mutation, logging without payload and keeping local keys without uploading a repair; corrupt local storage retains Decision 4’s legacy fallback.

`base` when the field is absent is the reader's local list, because the old writer did not see the spares and its record says nothing about them. When the field is present it is trusted as written, whether fresh from a new app or left on the server by an earlier new-app write that an old app's save did not touch. The same function handles both, so the reader never has to know which transport case it is in.

**Marking for re-upload after an old app's write.** Whenever an applied record has no list field, the reader enqueues one profile save after applying, in every case including a single-key or empty result, following the existing re-enqueue branch in `SyncApplyService` (`local_schema_newer_reenqueue`) and the whitelisted enqueue sites the commit guard enforces. The server record then carries the list again. The repair does not repeat: the reader's own write comes back with the field present, so this branch is not entered, and the apply path drops it as a payload-equal no-op (`equal_payload_noop`, `SyncApplyService.swift:411`; the last-modified-by self-echo filter applies to sessions only). Test 22 covers one repair followed by an echo that enqueues nothing.

**Fresh or reinstalled device.** A device that does not hold the profile has no local list to preserve, so it depends on the field being on the server record. It is, because an old app cannot remove it: outgoing records are built by `RecordProvider.materialize` from cached CloudKit system fields (`CKRecordSystemFieldsCodec`, `encodeSystemFields`, metadata only) and then `SyncedProfile.updateCKRecord` sets only the keys that app version knows; a fresh `CKRecord` is used only when no system fields are cached, and that record has no change tag, so the sync engine's save fails against an existing server record with `serverRecordChanged` (`SyncEngineController.swift:266`) and the record is refetched rather than overwritten. Records go through `CKSyncEngine`, whose saves use `ifServerRecordUnchanged` semantics: only keys set on the client record are written, and CloudKit removes a field only when a client explicitly sets it to nil. An old app never sets `physicalKeysData`, so the field survives every old-app edit, and a fresh device reads the full list. The only transient the orchestrator accepted is a fresh device syncing between an old app's write and the re-upload above, which by the same evidence does not occur; if device check 31 ever shows the field missing after an old-app edit, the merge is blocked until the cause is found. It does not downgrade to a release note.
...
22. Given a local profile with stop keys X and Y and an incoming record with no `physicalKeysData` and `stopNFCTagId` X, when applied, then the local stop list is still X and Y in that order, and exactly one profile save is enqueued. When the record that save produces (field present, same keys) is applied back, then nothing is enqueued and the apply is a payload-equal no-op. Given a local profile with no keys and the same absent-field record with `stopNFCTagId` nil, when applied, then one profile save is still enqueued.
...
26. Given an incoming record whose blob has X and Y and whose `stopNFCTagId` is nil, when applied, then the local stop list is still X and Y. Given an incoming record whose blob has an empty stop list and whose `stopNFCTagId` is nil, when applied, then the local stop list is empty even if it held keys before. An explicit empty list is the only path that clears a list; no editor action clears a stored id today.
...
31. On the old build, rename the profile. On the new build, confirm both keys are still present and that the server record carries `physicalKeysData` afterwards (visible in the CloudKit dashboard for the Development environment). A failure here blocks the merge.
