...
**Marking for re-upload after an old app's write.** If the field was absent and the reconciled lists differ from what the record alone would yield, the reader enqueues a profile save after applying, following the existing re-enqueue branch in `SyncApplyService` (`local_schema_newer_reenqueue`) and the whitelisted enqueue sites the commit guard enforces. The server record then carries the list again. The reader's own echo is dropped by the existing last-modified-by filter, so this cannot loop.
