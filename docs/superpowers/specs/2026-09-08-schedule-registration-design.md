...
| `Foqos/Views/BlockedProfileView.swift:668,782` and `Foqos/Models/BlockedProfiles.swift:739` | Duplicate is available whenever `!isBlocking`. `cloneProfile` copies `isManaged` and `managedByChildId`, saves the clone, and its caller registers it. | Child mode can create a locked clone, contrary to the mode matrix. Keep Duplicate available and create the Child-mode clone unlocked. |
...
Keep **Duplicate Profile** available under its existing active-session restriction, including for a locked source in Child mode. It does not require the lock code, and no `.duplicate` pending action is needed. In Child mode, construct the clone with `isManaged = false` and `managedByChildId = nil` inside `cloneProfile`, before persistence, snapshot publication, registration, or sync enqueue. Determine the current mode when cloning; make that input testable without adding an authorization service. Preserve the source profile's lock and ownership metadata and the existing Parent-mode cloning behavior.

This follows [App modes and locking](../../app-modes-and-locking.md): Child devices create only unlocked items. Duplicating a locked profile does not weaken its existing block; copying the lock instead creates unnecessary locked items and consumes schedule-registration capacity. The mode rule governs the new clone, not permission to invoke Duplicate.
...
4. Review the persistence entry points: `guard !editingDisabled else { return }` is the first statement of `saveProfile()`, the warning component has no callback parameter, and Duplicate has no added lock-code gate. Keep `ProfileEditGateTests` covering Child lock, temporary unlock, Parent, Individual, and active-session decisions for saving. The private SwiftUI actions have no direct test interface; do not add a view-testing dependency or a duplicate gate to test them.
5. Extend `CloneProfileTests` with a managed source and explicit Child mode. Assert the persisted clone has `isManaged == false` and `managedByChildId == nil`, its snapshot is unlocked, and the source's lock and ownership metadata are preserved. Cover an already-unlocked source and Parent-mode preservation of copied metadata. Use the existing in-memory model container and isolated SharedData defaults; these checks do not require family CloudKit setup.
6. When a managed Child profile with an active lock is available, perform a focused UI walkthrough with no temporary unlock. Confirm Fix Schedule is absent, normal Save remains hidden, and Duplicate opens its name prompt without verification and creates an unlocked clone. Confirm the existing card selection banner still opens the authorized editor flow, with no added editor instruction. If the required family CloudKit setup is unavailable, report the walkthrough as not run and complete checks 4 and 5; do not add test infrastructure or block the implementation on creating family setup.
7. Review and walk through the local save failure branches: registration and sync-enqueue warnings share the modal alert, both messages survive, and acknowledgment closes the editor without creating another profile. Do not claim these private view branches have automated coverage from utility tests.
8. Keep `ProfileAppSelectionStateTests`, `BlockedProfileSaveValidationTests`, and `ProfileSafetyOptionsTests` passing. Add the domain-only, selection-pending registration regression without changing the confirmation policy. Keep the shared scheduled-start refusal test in `ScheduleTimerActivityTests`.
9. Review session stop-registration failure for retained session state, error assignment after activation clears old errors, and uninterrupted activation bookkeeping. Verify deferred migration refreshes the snapshot before registration. Keep `StrategyManagerReconcileTests` and the existing snapshot tests passing.
10. Review the implementation diff for scope and run the normal gated build plus the changed test classes through the assigned build stream's `scripts/xcode-stream.sh` ownership. Do not run Xcode for this documentation-only PR.
