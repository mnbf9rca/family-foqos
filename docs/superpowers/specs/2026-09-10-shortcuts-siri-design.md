...
Add `ProfileStartTriggers.shortcuts` and enforce it inside `StrategyManager.startSessionFromBackground`, where saved identifiers and Siri requests converge. Require valid stop conditions and a stop route usable without a scanned start tag. Without a supplied duration, reuse the existing manual session lifecycle after those gates pass. Preserve the existing supplied-duration timer behavior where profile editing is unlocked; refuse that configuration write on a locked Child profile. Define the stop-reachability rule once for the available start credentials; separate issue [#493](https://github.com/mnbf9rca/family-foqos/issues/493) applies the same function to the Start button.
...
| `StartProfileIntent.perform`, `AppBlockerUtil.activateRestrictions`, `RequestAuthorizer` | A headless start has no authorization flow: manual start writes ManagedSettings, creates/publishes a session, and returns a success dialog without checking approved status or receiving an enforcement result. The timer branch can likewise publish a session before an OS registration failure is only logged. HomeView displays an authorization callout; a headless invocation does not. |
...
Keep the existing intent type, profile parameter identifier, UUID entity identity, and optional `durationInMinutes` parameter identifier. Apple documents no behavior for removing an existing parameter; retain its identifier without promising how saved shortcuts would load after removal. Describe Duration as optional and unavailable for locked Child profiles; no new spoken duration phrase.

A non-nil duration is a saved-profile edit. Before changing `strategyData`, `updatedAt`, the app-group snapshot, or the context, refuse it when `currentMode == .child && profile.isManaged`, with “You cannot change the duration of a locked profile with Siri or Shortcuts. Remove Duration to use its configured stop conditions.” Unlike `ProfileEditGate`, this intent guard deliberately ignores both `lockActive` (`canVerifyCode`) and temporary editor unlock: a missing Child code cache or an unlocked editor must not authorize this headless write. There is no intent-side lock-code prompt. A nil-duration start remains allowed on that profile if its start/stop rules allow it. Parent and Individual retain the edit access required by [the mode matrix](../../app-modes-and-locking.md); do not turn the managed marker into an all-mode lock gate.

For an editable profile, preserve today's bounds check (`DeviceActivityLimits.minimumIntervalMinutes` through `maximumTimerMinutes`), encode `StrategyTimerData`, write `strategyData` and `updatedAt`, update the snapshot, save the context, and select `ShortcutTimerBlockingStrategy`. Complete the eligibility checks and any awaits below before those writes. Do not ignore the duration, move it into a clone, or replace the existing timer strategy with a new lifecycle. An invalid duration fails before writes. A supplied valid duration provides its own timed stop through `StrategyTimerActivity`, independently of the profile's timer flag and `disableBackgroundStops`, as today; the human explicitly retained this behavior for editable profiles.
...
2. Profile exists and its trigger format is supported; any supplied duration passes the edit-lock guard and bounds check; no session is active. A repeated start does not toggle, restart, extend, or replace a session. A failed active-session fetch refuses the request.
3. Screen Time authorization is approved and the profile has device-local selection (`needsAppSelection == false`). Read current `AuthorizationCenter.shared.authorizationStatus` through the existing `AuthorizationRequesting` seam (`AuthorizationCenterRequester.shared` in production), accepting `.approved` and `.approvedWithDataAccess` as `RequestAuthorizer` does. Do not construct the view-owned authorizer, add a wrapper type, request authorization silently, or synthesize app-selection tokens. Keep this headless guard because the current path can publish a session and speak success without authorization or any enforcement result; refuse with an instruction to open the app and authorize Screen Time. This does not assert that a successful status check proves OS shielding, or change the tap path.
...
2. Extend `StrategyManagerBackgroundTests` using actual UUID lookup: manual-derived allowed, explicit disabled, tag/QR/schedule-only disabled, authorized opt-in alongside tags, empty stop conditions, same-tag-only and timer-only refusal without a supplied duration, usable alternatives, scan precedence and matching NFC/QR credentials in the shared predicate, background-stop restrictions, missing selection, denied Screen Time, active/repeated request, and stale/deleted entity. Test Child locked permitted start separately from forbidden configuration edits; Individual and Parent still work. Refusals leave session/configuration unchanged.
3. For supplied Duration on a locked Child profile, compare saved fields, `strategyData`, `updatedAt`, snapshot, and session count before/after refusal; nil duration can still start if eligible. For editable profiles, including Parent/Individual per the mode matrix, test both bounds and the accepted duration's saved data/snapshot, then verify selection of the existing timer strategy. Include timer-only and same-tag-only profiles with an accepted duration, proving that the actual timed branch supplies the exit while a mere saved timer flag does not. Preserve `stopConditions.isValid` refusal even with a duration. Diff-review that all eligibility gates precede duration writes and that only the nil-duration branch changes to `forceStart: false`; no new registrar API or transaction abstraction. Verify the same shared predicate is reserved for #493's tap caller, with no second validator. Preserve warnings already reported by the common lifecycle without asserting atomic rollback or guaranteed persistence it does not provide.
...
7. Before implementation is declared ready, complete the native-authentication device probe above, then use real Shortcuts/Siri for cold/warm process, locked/unlocked device, cancellation, selected/missing profile, duplicate names, accepted editable-profile durations, and refused locked-Child durations. Confirm actual restrictions and timer expiry for accepted durations, saved timer data only for those requests, and unchanged configuration for refusals; speech alone is insufficient. Exercise physical/geofence stop refusal and distinguish session/break status. Record exact OS/build, utterance, and result. If hardware is unavailable, report that gate to the orchestrator rather than claim it passed.
